...
class PerformanceEnv(aas_types.Environment):
    ...
    def build_nested_collection(self, current_depth: int, max_depth: int) -> aas_types.SubmodelElementCollection:
        """
        Build a SubmodelElementCollection that contains:
          1) A Property.
          2) (Optionally) Another SubmodelElementCollection if current_depth < max_depth.
        """
        # Create a base collection with a single Property for demonstration
        collection = aas_types.SubmodelElementCollection(
            id_short=f"SE_DEPTH_{current_depth}",
            value=[
                aas_types.Property(
        ...
        # If we still have depth to go, nest another collection
        if current_depth < max_depth:
            child_collection = self.build_nested_collection(current_depth + 1, max_depth)
            collection.value.append(child_collection)

        return collection

    def add_submodel_element_with_depth(self, depth: int = 1):
        """
        Create a TechnicalData submodel containing a nested SubmodelElementCollection
        structure up to the specified depth.
        """

        # Build our nested submodel collection structure
        #root_collection = self.build_nested_collection(current_depth=1, max_depth=depth)

        # As before, define other technical properties
        technical_properties = [
            aas_types.Property(
                id_short="Test",
                value_type=aas_types.DataTypeDefXSD.STRING,
            ...
            # Optionally, you could nest another multi-level collection here if desired
            self.build_nested_collection(current_depth=1, max_depth=depth),
        ]

        # Create your Submodel object
        submodel_with_submodel_collection = TechnicalData(
            new_id="test_id_short_lookup",
            general_information='general information',
            technical_properties=technical_properties
        )


        # Finally, attach the submodel to `self.submodels`
        self.submodels.append(submodel_with_submodel_collection)

    def getSubmodelCount(self):
